...
using EventsService.Aplicacion.Commands.Eventos.SubirFolletoEvento;
...
{
    public class SubirFolletoEventoHandler_Tests
    {
        private readonly Mock<IEventRepository> _mockEventRepo;
        private readonly Mock<IFileStorageService> _mockFileStorage;
        private readonly SubirFolletoEventoHandler _handler;
        ...
        public SubirFolletoEventoHandler_Tests()
        {
            _mockEventRepo = new Mock<IEventRepository>();
            _mockFileStorage = new Mock<IFileStorageService>();

            _handler = new SubirFolletoEventoHandler(
                _mockEventRepo.Object,
                _mockFileStorage.Object);

            _eventoId = Guid.NewGuid();
            _fileName = "folleto-test.pdf";
            _fileStream = new MemoryStream(new byte[] { 1, 2, 3, 4 });
            _expectedUrl = "https://cdn.test/eventos/folletos/folleto-test.pdf";
        }

        private SubirFolletoEventoCommand BuildCommand()
        ...
        #region Handle_Valido_DeberiaSubirFolleto_ActualizarEvento_YRetornarUrl()
        [Fact]
        public async Task Handle_Valido_DeberiaSubirFolleto_ActualizarEvento_YRetornarUrl()
        {
            // ARRANGE
            ...
            Assert.Equal(_expectedUrl, eventoActualizado!.FolletoUrl);

            _mockEventRepo.Verify(r => r.GetByIdAsync(_eventoId, It.IsAny<CancellationToken>()), Times.Once);
            _mockFileStorage.Verify(s => s.UploadFileAsync(
                    command.FileStream,
                    command.FileName,
                    It.IsAny<CancellationToken>()),
                Times.Once);
        ...
        public async Task Handle_FalloEnStorage_DeberiaPropagarExcepcion()
        {
            ...
            var storageException = new InvalidOperationException("Error subiendo folleto a storage.");
            _mockFileStorage
                .Setup(s => s.UploadFileAsync(
                    command.FileStream,
                    command.FileName,
            ...
            Assert.Equal("Error subiendo folleto a storage.", ex.Message);
